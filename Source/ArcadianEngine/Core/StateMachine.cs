...
public class StateMachine<TG>(string stateMachineName, GameContext<TG> cx)
    where TG : ArcadianGame<TG>
{
    ...
    public virtual void AddState<T>(T state)
        where T : State<TG>
    {
    ...
    {
        if (States.Count == 0)
            throw new EmptyStateMachineException(StateMachineName);
        if (!Initialized)
            throw new StateMachineNotInitializedException(StateMachineName);
    }

...
public sealed class LinearStateMachine<TG>(string stateMachineName, GameContext<TG> cx)
    : StateMachine<TG>(stateMachineName, cx)
    where TG : ArcadianGame<TG>
{
    ...
    {
        if (!States.ContainsKey(stateName) || stateName == "")
            throw new StateNotFoundException(StateMachineName, stateName);
        if (stateName == CurrentStateName)
            throw new EmptyStateNameException(StateMachineName);
